...
describe('Night Actions Service', () => {
  ...
  describe('advanceNight', () => {
    ...
    it('sets nightActions.cupidLinks to the provided value, but only sets lovers if count is 2', async () => {
      ...
      await nightActions.advanceNight(testGameState1, players, now, 'cupidLinks', [
        'p1',
        'p2',
        'p3',
      ]);

      expect(testGameState1.update).toHaveBeenCalledWith(
        expect.objectContaining({
      ...
      const testGameState2 = new MockGameState({
        // Create a new instance for the second call
        ...mockGameStateInstance._state,
        phase: PHASES.NIGHT_CUPID,
        nightActions: { cupidLinks: [] },
        lovers: [],
      ...
      expect(testGameState2.update).toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    it('skips NIGHT_CUPID if no Cupid is present or lovers are already set', async () => {
      ...
      let playersWithCupid = [
        { ..._mockPlayersArray[0], role: ROLE_IDS.CUPID },
        ..._mockPlayersArray.slice(1),
      ];
    ...
    it('advances to RESOLVE phase and calls resolveNight when no more night actions', async () => {
      ...
      vi.spyOn(winConditions, 'checkWinCondition').mockReturnValueOnce({
        isGameOver: true,
        winner: 'VILLAGERS',
        winners: ['VILLAGERS'],
      });
  ...
  describe('resolveNight', () => {
    ...
    it('eliminates vigilante target if not protected', async () => {
      ...
      const playersWithVigilante = [vigilantePlayer, targetPlayer, ..._mockPlayersArray.slice(2)];
    ...
    it('handles lover chain death when the second lover dies first', async () => {
      ...
      const playersWithLovers = [werewolfPlayer, lover1, lover2, ..._mockPlayersArray.slice(3)];
    ...
    it('handles tie in werewolf votes by randomly selecting a target', async () => {
      ...
      await nightActions.resolveNight(
        testGameState,
        playersWithWolvesAndTargets,
        localNightActionsState
      );
